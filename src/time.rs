...
pub fn normalize_time(&t: &Time) -> (Time, f64) {
    let (sec, min_excess): (f64, f64) = reduce_to_exclusive(t.sec, 60.0);

    let min: f64 = (t.min as f64) + min_excess;
    let (min, hour_excess): (f64, f64) = reduce_to_exclusive(min, 60.0);

    let hour: f64 = (t.hour as f64) + hour_excess;
    let (hour, day_excess): (f64, f64) = reduce_to_exclusive(hour, 24.0);
...
pub fn gst_from_ut(&ut: &DateTime) -> Time {
    ...
    let (t0, _factor) = reduce_to_exclusive(t0, 24.0);

    let mut decimal = decimal_hours_from_time(&Time::from(&ut));
    decimal *= 1.002_737_909;
    decimal += t0;

    let (decimal, _factor): (f64, f64) = reduce_to_exclusive(decimal, 24.0);
...
pub fn ut_from_gst(&gst: &DateTime) -> Time {
    ...
    let t = s / 36_525.0;
    let t0 = 6.697_374_558 + (2_400.051_336 * t) + (0.000_025_862 * t * t);
    let (t0, _factor): (f64, f64) = reduce_to_exclusive(t0, 24.0);

    let decimal = decimal_hours_from_time(&Time::from(&gst));
    let (decimal, _factor2): (f64, f64) = reduce_to_exclusive(decimal - t0, 24.0);
...
mod tests {
    ...
    #[test]
    fn day_number_from_date_works_0() {
        let date = Date {
            year: 1988,
            month: Month::Jul,
            day: 27.0,
    ...
    #[test]
    fn decimal_hours_from_time_works() {
        let time = Time {
            hour: 18,
            min: 31,
            sec: 27.0,
    ...
    #[test]
    fn time_from_decimal_hours_works() {
