/// Polynomial expressions for "delta-T" provided
/// by NASA
///
/// When calculating the motion of the moon, UT
/// (Universal Time) is not accurate enough, but needs
/// TDT (Terrestrial Dynamical Time). Earth's spin
/// around the polar axis varies in time, and NASA
/// provides a look-up tables for finding, so called,
/// "delta-T" (ΔT). Yet, NASA provides simplified
/// arithmetic expressions as well. Here, the program
/// contains a set of functions that are translated
/// into Rust programs.
///
/// References:
///
...
/// Between years -500 and +500, we use the data from
/// Table 1, except that for the year -500 we changed
/// the value 17190 to 17203.7 in order to avoid
/// a discontinuity with the previous formula at that
/// epoch. The value for ΔT is given by a polynomial
/// of the 6th degree, which reproduces the values in
...
/// Between years +500 and +1600, we again use the
/// data from Table 1 to derive a polynomial of
/// the 6th degree.
///
...
fn get_ad1600_to_ad1700(year: f64) -> f64 {
    let t = year - 1600.0;
    120.0 - 0.9808 * t - 0.01532 * t.powi(2)
        + t.powi(3) / 7129.0
}
...
fn get_ad1920_to_ad1941(year: f64) -> f64 {
    let t = year - 1920.0;
    21.20 + 0.84493 * t - 0.076100 * t.powi(2)
        + 0.0020936 * t.powi(3)
}
...
fn get_ad2050_to_ad2150(year: f64) -> f64 {
    -20.0 + 32.0 * ((year - 1820.0) / 100.0).powi(2)
        - 0.5628 * (2150.0 - year)
}
